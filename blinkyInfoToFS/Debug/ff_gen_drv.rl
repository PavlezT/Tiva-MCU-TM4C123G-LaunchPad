...
N  * @brief   FatFs generic low level driver.
N  ******************************************************************************
N  * @attention
N  *
...
N/* Private define ------------------------------------------------------------*/
N/* Private variables ---------------------------------------------------------*/
NDisk_drvTypeDef disk = {{0},{0},{0},0};
N
N/* Private function prototypes -----------------------------------------------*/
N/* Private functions ---------------------------------------------------------*/
N
N/**
N  * @brief  Links a compatible diskio driver/lun id and increments the number of active
N  *         linked drivers.
N  * @note   The number of linked drivers (volumes) is up to 10 due to FatFs limits.
N  * @param  drv: pointer to the disk IO Driver structure
N  * @param  path: pointer to the logical drive path
N  * @param  lun : only used for USB Key Disk to add multi-lun management
N            else the paramter must be equal to 0
N  * @retval Returns 0 in case of success, otherwise 1.
N  */
Nuint8_t FATFS_LinkDriverEx(Diskio_drvTypeDef *drv, char *path, uint8_t lun)
N{
N  uint8_t ret = 1;
N  uint8_t DiskNum = 0;
N
N  if(disk.nbr <= _VOLUMES)
X  if(disk.nbr <= 1)
N  {
N    disk.is_initialized[disk.nbr] = 0;
N    disk.drv[disk.nbr] = drv;
N    disk.lun[disk.nbr] = lun;
N    DiskNum = disk.nbr++;
N    path[0] = DiskNum + '0';
N    path[1] = ':';
N    path[2] = '/';
...
N
N/**
N  * @brief  Links a compatible diskio driver and increments the number of active
N  *         linked drivers.
N  * @note   The number of linked drivers (volumes) is up to 10 due to FatFs limits
N  * @param  drv: pointer to the disk IO Driver structure
N  * @param  path: pointer to the logical drive path
...
Nuint8_t FATFS_LinkDriver(Diskio_drvTypeDef *drv, char *path)
N{
N  return FATFS_LinkDriverEx(drv, path, 0);
N}
N
N/**
N  * @brief  Unlinks a diskio driver and decrements the number of active linked
N  *         drivers.
N  * @param  path: pointer to the logical drive path
N  * @param  lun : not used
N  * @retval Returns 0 in case of success, otherwise 1.
N  */
Nuint8_t FATFS_UnLinkDriverEx(char *path, uint8_t lun)
N{
N  uint8_t DiskNum = 0;
N  uint8_t ret = 1;
N
...
N  {
N    DiskNum = path[0] - '0';
N    if(disk.drv[DiskNum] != 0)
N    {
N      disk.drv[DiskNum] = 0;
N      disk.lun[DiskNum] = 0;
N      disk.nbr--;
N      ret = 0;
N    }
N  }
N
N  return ret;
R "../ff_gen_drv.c" 116 13 (ULP 5.4) Detected an assignment to a type with size less than int. To avoid unnecessary sign extension, use int-sized types for local varaibles and convert to smaller types for static storage.
N}
N
N/**
N  * @brief  Unlinks a diskio driver and decrements the number of active linked
...
Nuint8_t FATFS_UnLinkDriver(char *path)
N{
N  return FATFS_UnLinkDriverEx(path, 0);
N}
N
N/**
N  * @brief  Gets number of linked drivers to the FatFs module.
N  * @param  None
N  * @retval Number of attached drivers.
N  */
Nuint8_t FATFS_GetAttachedDriversNbr(void)
N{
N  return disk.nbr;
